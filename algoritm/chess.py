...
class Knight(Figure):
    def can_go(self, x, y, color, desk):
        return (self.x, self.y) in [(x - 2, y - 1), (x - 2, y + 1), (x + 2, y - 1), (x + 2, y + 1), (x + 1, y + 2),
                                    (x + 1, y - 2),
                                    (x - 1, y - 2), (x - 1, y + 2)]
class Bishop(Figure):
...
while True:
    ...
    try:
        data = input().split()
        x,y= map(int,list(data[0]))
        to_x,to_y = map(int,list(data[1]))
        DESK0.go_to(x,y,to_x,to_y)
    except:
        pass
