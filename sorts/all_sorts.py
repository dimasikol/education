...
class Sorts:
    ...
    @staticmethod
    def radix_sort(array: list[int], clean_func=False) -> list[int]:
        if clean_func:
            array = array.copy()
    ...
    @staticmethod
    def counter_sort(array: list[int]) -> list[int | str | float]:
        _data = {}
        mx = mn = array[0]
        for i in range(len(array)):
            _data[array[i]] = _data.get(array[i], 0) + 1
            if mx < array[i]:
                mx = array[i]
            if mn > array[i]:
                mn = array[i]
    ...
    @staticmethod
    def cube_sort(arr: list[int]) -> list[int]:
        data = {}
        result = []
        for num in arr:
            pref = 1
        ...
        for sq3 in Sorts.counter_sort(data_3):
            data_2 = [i for i in data[sq3]]
            for sq2 in Sorts.counter_sort(data_2):
                result += Sorts.counter_sort(data[sq3][sq2])
        return result
    ...
    @staticmethod
    def bucket_sort(array:list[int | str | float]):
        if len(array) == 2:
        ...
        return Sorts.bucket_sort(bucket_min) + bucket_mid + Sorts.bucket_sort(bucket_max)
    ...
    def testing_all_sorts(mass: list[int | str | float], res):
        funcs = [Sorts.buble_sort, Sorts.selection_sort, Sorts.insertion_sort, Sorts.shake_sort, Sorts.gnome_sort,
                 Sorts.shell_sort, Sorts.heap_sort, Sorts.radix_sort, Sorts.merge_sort, Sorts.quick_sort, Sorts.bucket_sort, Sorts.cube_sort, Sorts.counter_sort]
...
if __name__ == '__main__':
    ...
    data = [random.randint(-100, 100) for i in range(10000)]
    Sorts.testing_all_sorts(data, proxy)
    for _id, i in enumerate(proxy, start=1):
        print(f"{_id}.{proxy[i]['name']}".rjust(16," "), proxy[i]['result'], round(proxy[i]['time'],4),)
